...
const CUSTOMER_CREATE_UPDATE_EXTENSION_KEY =
  'myconnector-customerCreateUpdateExtension';
...
export async function createCustomerCreateOrUpdateExtension(
  apiRoot: ByProjectKeyRequestBuilder,
  applicationUrl: string
): Promise<void> {
  const {
    ...
    .get({
      queryArgs: {
        where: `key = "${CUSTOMER_CREATE_UPDATE_EXTENSION_KEY}"`,
      },
    })
    .execute();
  ...
  if (extensions.length > 0) {
    ...
    await apiRoot
      .extensions()
      .withKey({ key: CUSTOMER_CREATE_UPDATE_EXTENSION_KEY })
      .delete({
        queryArgs: {
          version: extension.version,
        },
  ...
  await apiRoot
    ...
    .post({
      body: {
        key: CUSTOMER_CREATE_UPDATE_EXTENSION_KEY,
        destination: {
          type: 'HTTP',
          url: applicationUrl,
        },
...
export async function deleteCustomerCreateOrUpdateExtension(
  apiRoot: ByProjectKeyRequestBuilder
): Promise<void> {
  const {
    body: { results: extensions },
